...
def db_connect():
    ...
    try:
        ...
        dbCursor.execute('''CREATE TABLE filmimage (idfilmimage INT PRIMARY KEY AUTO_INCREMENT,
         imagefilm LONGBLOB, idkinopoiskfilm INT,reviewdescr LONGTEXT)
         DEFAULT CHARSET=utf8 DEFAULT COLLATE utf8_unicode_ci''')
...
def enter_film_name():
    serch_name = str(
        raw_input('Enter film name or if you want enter studio click [ENTER]: '))
    if serch_name:
...
def insert_data(db, data, studio):
    ...
    dbCursor.execute(
        ...
        (img_requests, str(idkinopoiskfilm), film_res))
    dbCursor.execute('COMMIT')
    db.close()
...
def main():
    ...
    print '_________________________END________________________'


if __name__ == '__main__':
    print '_____________________________START PARSER ________________________________'
    main()
